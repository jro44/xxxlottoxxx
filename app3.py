#=================================================================
#Ta Aplikacja ma dodatkowy tryb łączenia wszystkich trybów w jeden 
#Oraz filtracji przez kandytatów. Najbardziej pro
#NAJLEPIEJ UŻYWAĆ NA PC.
#================================================================
...
def generate_ultimate_next_draw(
    memory_df,
    pair_df,
    transition_df,
    move_df,
    ...
    if not ranked:
        raise ValueError("Ultimate engine could not generate candidates.")
...
def create_report(
    ...
    for i, item in enumerate(generated, 1):
        ...
        lines.append(
            f"{i}. {nums} | POWER={item['score']} | MODE={source_mode}"
        )
...
st.markdown(
    ...
    ✔ History treated from newest to oldest<br>
    ✔ ULTIMATE NEXT DRAW mode
    </div>
    """,
    unsafe_allow_html=True
)
...
if st.button(
    ...
    if mode == "ULTIMATE NEXT DRAW":
        ranked_results = generate_ultimate_next_draw(
            memory_df,
            pair_df,
            transition_df,
            move_df,
        ...
        generated_results = ranked_results[:results_count]
